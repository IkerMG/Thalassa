"""
Scraper para Tiendanimal (tiendanimal.es).

Plataforma: Salesforce Commerce Cloud (misma base que Kiwoko).
...
_BASE_URL = "https://www.tiendanimal.es"
...
async def scrape_tiendanimal(keyword: str) -> list[ProductResult]:
    """
    Realiza la búsqueda en Tiendanimal y devuelve una lista de ProductResult.
    Lanza excepciones para que scraper_service las capture.
    """
    url = f"{_BASE_URL}{_SEARCH_PATH}?q={quote(keyword)}&lang=default"
    print(f"[tiendanimal] GET {url}")

    async with httpx.AsyncClient(timeout=_TIMEOUT, follow_redirects=True) as client:
        response = await client.get(url, headers=_HEADERS)
        print(f"[tiendanimal] status_code={response.status_code}")
    ...
    if not cards:
        logger.warning("Tiendanimal: 0 productos encontrados para '%s'", keyword)
        return []
    ...
    for card in cards:
        try:
            ...
            # Precio — data-min-price ya es un float serializado ("89.99")
            ...
            logger.debug("Tiendanimal: error parseando un producto — %s", exc)
            continue

    print(f"[tiendanimal] {len(results)} productos parseados")
    logger.info("Tiendanimal: %d productos encontrados para '%s'", len(results), keyword)
    return results
