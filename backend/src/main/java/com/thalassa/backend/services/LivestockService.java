...
public class LivestockService {
  ...
  public List<LivestockResponse> listLivestock(Long aquariumId) {
    ...
    return livestockRepository.findByAquariumId(aquariumId).stream()
        .map(this::mapToResponse)
        .toList();
  }

  /**
   * Añade un espécimen. Si el acuario es REEF y el espécimen no es reef-safe, se incluye un campo
   * warning en la respuesta (sin bloquear).
   */
  @Transactional
  public AddLivestockResponse addLivestock(Long aquariumId, LivestockRequest request) {
    ...
    if (request.getSpeciesCatalogId() != null) {
      species =
          speciesCatalogRepository
              .findById(request.getSpeciesCatalogId())
              .orElseThrow(
                  () -> new ResourceNotFoundException("Especie del catálogo no encontrada."));
    }
    ...
    return AddLivestockResponse.builder().livestock(mapToResponse(saved)).warning(warning).build();
  }
  ...
  public LivestockResponse updateLivestock(Long livestockId, LivestockRequest request) {
    ...
    Livestock livestock =
        livestockRepository
            .findByIdAndAquariumUserId(livestockId, user.getId())
