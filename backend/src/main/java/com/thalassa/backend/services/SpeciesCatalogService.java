...
public class SpeciesCatalogService {
  ...
  /**
   * Busca por nombre común o científico (case-insensitive). Query vacía/nula devuelve el catálogo
   * completo.
   */
  public List<SpeciesCatalogResponse> search(String query) {
    String q = (query == null) ? "" : query.trim();
    return speciesCatalogRepository.searchByName(q).stream().map(this::mapToResponse).toList();
  }
  ...
  public SpeciesCatalogResponse getById(Long id) {
    SpeciesCatalog species =
        speciesCatalogRepository
            .findById(id)
            .orElseThrow(
                () -> new ResourceNotFoundException("Especie no encontrada en el catálogo."));
