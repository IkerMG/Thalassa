...
public interface SpeciesCatalogRepository extends JpaRepository<SpeciesCatalog, Long> {
  ...
  /**
   * Búsqueda case-insensitive por nombre común o nombre científico. Con query vacía ("") devuelve
   * todo el catálogo (LIKE '%%' siempre es true).
   */
  @Query(
      "SELECT s FROM SpeciesCatalog s WHERE "
